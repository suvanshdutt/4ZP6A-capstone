...
class CheXpert(Dataset):
    ...
    def _read_listfile(self, listfile: Path) -> tuple[list[str], list[Tensor]]:
        ...
        # TODO: Replace -1 with 0 and fill NaN with 0 for NOW
        labels = data[CheXpert.CLASSES].replace(-1, 0).fillna(0).values
    ...
    def _generate_full_patient_paths(self, paths: list[str]) -> list[str]:
        ...
        for p, path in enumerate(paths):
            ...
            if patient_id < 21514:
                ...
                paths[p] = (
                    str(self.DATA_DIR.absolute())
                    + "/CheXpert-v1.0 batch 1 (validate & csv)/CheXpert-v1.0 batch 1 (validate & csv)/valid/"
                    + path
                )
    ...
    def _get_image_paths_labels(self) -> tuple[list[str], list[Tensor]]:
        ...
        """
        if self.train:
            listfile = self.DATA_DIR / "train_cheXbert.csv"
        else:
            listfile = (
                self.DATA_DIR
...
if __name__ == "__main__":
    ...
    # print(len(dataset))
    # img, label = dataset[0]
    # print(CheXpert.get_decoded(label))
